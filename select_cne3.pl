...
chdir("$DIR_NAME");
system("mkdir match_exon");
system("mkdir match_NA");
open(OUT,">$FILE_NAME");
...
print OUT "Result id";
print OUT "\tRef id\tExons match\tRef start\tRef end\tStrand\tIdentity\tHit id start end\tHit organism" x scalar(keys %$species);
print OUT "\n";
...
print "Here are the type of feature in the DB:\n\n";
print $_."\n" foreach $ref_db->types;
print "\n";
...
foreach my $feature(@features) {
  ...
  my @fos0 = $ref_db->features(-seq_id => $feature->seq_id,
                               -start => $feature->start,
                               -end => $feature->end,
                               -type => 'tRNA');
  next if scalar(@fos0);

  # Check to filter out matches overlapping noncoding exons
  my $next = 0;
  my $exon = 0;
  ...
  foreach my $res(@res) {
    ...
    $print .= $res->seq_id."\t".($exon||'NA')."\t".$res->start."\t".$res->end."\t".$res->strand."\t".$res->score."\t".$T[0]."\t".$O[0]."\t";
    $ref_id = $res->seq_id;
  ...
  my $eval = 'match_NA';
  $eval = 'match_exon' if $exon;
  my $fasta = Bio::SeqIO->new(-file => ">$eval\/$nr\.fa",
                              -format => 'fasta');
